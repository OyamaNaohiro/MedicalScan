...
enum ScanMode: Int {
    ...
    func makeConfig() -> ScanConfig {
        ...
        case .hand:
            ...
            // ×6(9mm)は癒着、×3(4.5mm)は薄すぎて周回で細部が途切れ・ぼこぼこ。×3.5(5.25mm)を採用。
            c.truncationScale = 3.5
            ...
            // 側面・細部は周回中に短時間しか観測されず重みが伸びない。閾値を下げて少ない観測でも
            // 面化させ、途切れを繋ぐ。ビルド100の 2.0 はぼこぼこだったので中間の 2.5 で刻む。
            c.meshMinWeight = 2.5    // 既定3→2.5
        case .foot:
