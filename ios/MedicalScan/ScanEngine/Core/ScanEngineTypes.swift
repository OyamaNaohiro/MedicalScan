...
enum ScanMode: Int {
    ...
    func makeConfig() -> ScanConfig {
        ...
        case .hand:
            ...
            // 1.5mm はノイズが目立ちやすい。帯は薄いまま「ボリューム空間の平均化」で表面を滑らかに
            // する（指の隙間＝空間エッジは保つ）。temporalAlpha は残像が出るため既定(0.2)のまま。
            c.maxWeight = 128        // ボリューム空間での観測平均を増やす（64→128）
            // 薄い truncation は視点間の融合のりしろも狭い。周回中に細部を拾って繋げるため、
            // 表面化しきい値を下げ、斜め視点の観測も拾う（truncation は触らず指の分離は維持）。
            c.meshMinWeight = 2      // 観測の少ない細部も表面化（3→2）
            c.grazingCosMin = 0.05   // 指の側面・縁など斜め視点を拾う（0.1→0.05）
        case .foot:
